  ...
  return (
    <section className={`cs-hero relative overflow-hidden bg-ploy-background-primary px-5 pb-16 pt-28 sm:px-8 sm:pb-20 sm:pt-32 lg:px-10 ${editorial ? "" : "flex min-h-screen items-end"}`}>
      ...
      <div className="cs-hero__inner relative mx-auto w-full max-w-[92rem]">
        ...
        {summary && <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ ...transition, delay: 0.14 }} className={`cs-hero__summary mt-8 leading-relaxed text-ploy-text-secondary ${editorial ? "max-w-3xl text-xl sm:text-2xl" : "max-w-2xl text-lg"}`}>{summary}</motion.p>}
        {meta.length > 0 && (
